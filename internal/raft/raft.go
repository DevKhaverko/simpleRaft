...
func (s *Server) persist(writeLog bool, nNewEntries int) {
	...
	ServerAssert(s, "Wrote full page", n, PageSize)

	if writeLog && nNewEntries > 0 {
		newLogOffset := max(len(s.log)-nNewEntries, 0)

		_, err := s.fd.Seek(int64(PageSize+EntrySize*newLogOffset), 0)
		if err != nil {
			panic(err)
		}
		bw := bufio.NewWriter(s.fd)

		var entryBytes [EntrySize]byte
		for i := newLogOffset; i < len(s.log); i++ {
			// 0 - 8:    Entry term
			// 8 - 16:   Entry command length
			// 16 - ENTRY_SIZE: Entry command

			if len(s.log[i].Command) > EntrySize-EntryHeader {
				panic(fmt.Sprintf("Command is too large (%d). Must be at most %d bytes.", len(s.log[i].Command), EntrySize-EntryHeader))
			}
			...
			ServerAssert(s, "Wrote full page", n, EntrySize)
		}
...
func (s *Server) restore() {
	...
	ServerAssert(s, "Read full page", n, PageSize)
	...
	if lenLog > 0 {
		_, err := s.fd.Seek(int64(PageSize), 0)
		if err != nil {
			panic(err)
		}
		...
		for i := 0; uint64(i) < lenLog; i++ {
			...
			ServerAssert(s, "Read full entry", n, EntrySize)
...
- если в ответе сервера терм больше, чем у текущего, то текущий сервер становится follower
- если терм ответа не совпадает с термом запроса, то его нельзя считать актуальным, поэтому он не идет в расчет
- устанавливаем голос от сервера за текущий сервер
*/
...
/*
Принцип работы:
- если пришедший запрос на голос имеет терм больше терма текущего сервера, то текущий сервер становится follower
- если пришедший запрос на голос имеет терм меньше терма текущего сервера, то это старый запрос, и голосовать за тот сервер не надо
- проверяем, что лог сервера, который запросил голос, хотя бы такой же как у текущего сервера, и что текущий сервер не голосовал за себя
*/
...
/*
Принцип работы:
- если пришедший запрос  имеет терм больше терма текущего сервера, то текущий сервер становится follower
- если текущий сервер является кандидатом, и терм запроса совпадает с текущим термом сервера, значит сервер получил appendEntries от лидера
и он становится follower
- если пришедший запрос имеет терм меньше терма текущего сервера, то это запрос от старого лидедра, и его нужно дропнуть
- сбрасываем таймаут перевыборов, так как лидер есть, и он доступен
...
- асинхронно рассылаем сообщения остальным серверам
- берем только ту часть лога текущего сервера, которой нет у другого сервера
- если в полученном ответе терм большего, чем у текущего сервера, то становимся follower
- если термы запроса и ответа не совпадают, то данный ответ является не актуальным и можно его не учитывать
- если ответ от сервера успешный, то мы устанавливаем его стейт как у текущего сервера
- если ответ сервера неуспешный, то надо откатываться назад, и искать, где произошло расхождение
*/
...
/*
Принцип работы:
- проверяем случился ли таймаут выбора лидера
- если не случился, то ничего не делаем
- если случился, то сервер становится кандидатом, увеличивается текущий терм сервера,
сервер голосует за себя, а голоса остальных выставляет в 0, чтобы потом их обновить
- устанавливается новый таймаут для выбора лидера(сделано для того, чтобы сервера не попадали в один и тот же
таймаут, и не было бесконечных выборов)
- текущее состояние записывается на диск
- запрашиваются голоса у других серверов
*/
...
/*
Принцип работы:
Если кол-во голосов за текущий сервер ≥ кворума, то
- устанавливаем позицию для следующего индекса у всех серверов равной длине лога текущего сервера + 1
- устанавливаем matchIndex в ноль у всех членов кластера
(всегда после выборов лидера стейт сбрасывается, так как лидер накатывает свой стейт)
*/
...
/*
Принцип работы:
Если наступило время для отправки хартбита, то шлем его.
